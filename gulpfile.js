...
gulp.task('pre-build', ['html', 'images', 'fonts','extras'], () => {
  return gulp.src('build/**/*').pipe($.size({title: 'BUILD TASK DONE END SIZE', gzip: true}));
});

gulp.task('pre-build-minify', ['html-minify', 'images', 'fonts','extras'], () => {
  return gulp.src('build/**/*').pipe($.size({title: 'BUILD MINIFY!!! TASK DONE END SIZE', gzip: true}));
});
